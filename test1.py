...
# Sample CSS styles and labels (Replace with your actual data)
css_data = ["""<!-- HTML !-->
...
def modify_css_with_ml(css):
    # Rule-based algorithm to add a new CSS property to all buttons
    modified_css = css.replace('.button-', '.button-modified-')
    modified_css += """
.button-modified {
  border-radius: 5px;
...
# Labels indicating the type of modification (0 - No modification, 1 - Color change, 2 - Remove, etc.)
labels = [0, 1]  # Example labels for the first two CSS styles
...
zipped_data = zip_longest(css_data, labels, fillvalue='')

# Unzip the zipped_data
css_data, labels = zip(*zipped_data)
modified_css_data = [modify_css_with_ml(css) for css in css_data]
# Tokenize the CSS styles and convert labels to one-hot encoded tensors
inputs = tokenizer(css_data, truncation=True, padding=True, return_tensors='pt')
labels = torch.tensor(pd.get_dummies(labels, columns=[0, 1, 2, 3]).values, dtype=torch.float32)
